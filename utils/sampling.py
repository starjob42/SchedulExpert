...
class JobShopStates:
    ...
    #for monte carlo sequential search
    def copy_state_from(self, other):
        """
        Fast deep copy of the dynamic state tensors from another instance.
        Used to clone the environment for proxy rollouts without overhead.
        """
        # Copy scalar attributes
        self.num_j = other.num_j
        self.num_m = other.num_m
        self.bs = other.bs
        self._factor = other._factor
        self._eps = other._eps
        
        # Reference static tensors (no copy needed, they are constant)
        self.machines = other.machines
        self.costs = other.costs
        self._bs_idx = other._bs_idx
        self.j_st = other.j_st
        
        # Deep copy dynamic tensors (The actual state)
        # We use .clone() to ensure memory independence
        self.j_idx = other.j_idx.clone()
        self.j_ct = other.j_ct.clone()
        self.m_ct = other.m_ct.clone()

    def reorder_particles(self, idx):
        """
        Reorders the internal batch dimension based on resampling indices.
        Used to kill bad particles and duplicate good ones in-place.
        """
        # idx is a LongTensor of shape (bs,)
        self.j_idx = self.j_idx.index_select(0, idx)
        self.j_ct = self.j_ct.index_select(0, idx)
        self.m_ct = self.m_ct.index_select(0, idx)
    ...
    def _ensure_buf_names(self):
        if hasattr(self, '_buf_names') and self._buf_names:
            return
        self._buf_names = []
        for k, v in self.__dict__.items():
            if isinstance(v, torch.Tensor) and v.dim() >= 1 and v.size(0) == self.batch_size:
                self._buf_names.append(k)

    def clone(self):
        self._ensure_buf_names()
        new = copy.copy(self)
        for k in self._buf_names:
            setattr(new, k, getattr(self, k).clone())
        return new

    def gather(self, idx):
        """Select a subset of beams by index (1D LongTensor)."""
        self._ensure_buf_names()
        for k in self._buf_names:
            buf = getattr(self, k)
            setattr(self, k, buf.index_select(0, idx))
        self.batch_size = idx.numel()
        return self

    def tile(self, K: int):
        """Repeat each beam K times (repeat_interleave along batch dim)."""
        self._ensure_buf_names()
        for k in self._buf_names:
            buf = getattr(self, k)
            setattr(self, k, buf.repeat_interleave(K, dim=0))
        self.batch_size *= K
        return self
