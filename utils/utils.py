...
# New class to compute average processing time per instance shape
class TimeObjMeter:
    def __init__(self):
        self.sum = {}
        self.count = {}

    def update(self, ins: dict, elapsed: float):
        """
        Update with the elapsed processing time for a given instance shape.

        Args:
            ins: JSP instance.
            elapsed: processing time for the instance.
        """
        shape = ins['shape']
        if shape not in self.sum:
            self.sum[shape] = elapsed
            self.count[shape] = 1
        else:
            self.sum[shape] += elapsed
            self.count[shape] += 1
    ...
    def __str__(self):
        out = ""
        for shape in sorted(self.sum):
            avg_time = self.sum[shape] / self.count[shape]
            out += f"\t\t\t{shape:5}: AVG Time = {avg_time:4.3f} seconds\n"
        return out[:-1]

    @property
    def avg(self):
        total_time = sum(self.sum.values())
        total_count = sum(self.count.values())
        return total_time / total_count if total_count else 0
